...
def calculate_psnr_mask(img, img2, mask, crop_border, input_order='HWC', test_y_channel=False, **kwargs):
    ...
    if img.ndim == 2:  # (H, W)の場合
        img = img[:, :, np.newaxis]  # (H, W) -> (H, W, 1)
    if img2.ndim == 2:  # (H, W)の場合
        img2 = img2[:, :, np.newaxis]  # (H, W) -> (H, W, 1)
    if mask.ndim == 2:  # (H, W)の場合
        mask = mask[:, :, np.newaxis]  # (H, W) -> (H, W, 1)
    #mask = mask[..., 0]
    if img.ndim == 4:  # (N, C, H, W)
        img = np.transpose(img[0], (1, 2, 0))  # (H, W, C)
    if img2.ndim == 4:
        img2 = np.transpose(img2[0], (1, 2, 0))  # (H, W, C)
    if mask.ndim == 4:
        mask = mask[0, 0]  # (H, W)
    elif mask.ndim == 3:
        mask = mask[0]  # (H, W)
    ...
    masked_img = img[..., 0][mask != 0]  # 1チャネル目を選択
    masked_img2 = img2[..., 0][mask != 0]  # 1チャネル目を選択    
    # print("masked_img stats:")
    # print("  min:", np.min(masked_img))
    # print("  max:", np.max(masked_img))
    # print("  mean:", np.mean(masked_img))
    # print("  shape:", masked_img.shape)
    # print("  dtype:", masked_img.dtype)
    # print("  non-zero count:", np.count_nonzero(masked_img))
    # print("masked_img2 stats:")
    # print("  min:", np.min(masked_img2))
    # print("  max:", np.max(masked_img2))
    # print("  mean:", np.mean(masked_img2))

    mse = np.mean((masked_img - masked_img2)**2)
    if mse == 0:
        return 0.0, 0
    return 20. * np.log10(255. / np.sqrt(mse)), 1
...
def _ssim_mask(img, img2, mask):
    ...
    masked_ssim_map = ssim_map[boolean_mask[5:-5, 5:-5]]
    return masked_ssim_map.mean()

def calculate_ssim_mask(img, img2, mask, crop_border, input_order='HWC', test_y_channel=False, **kwargs):
    ...
    return _ssim_mask(img, img2, mask), 1
