...
def paired_random_crop_mask_label(img_gts, img_lqs, img_masks, img_labels, gt_patch_size, scale, gt_path=None):
    """Paired random crop. Support Numpy array and Tensor inputs.
    ...
    if input_type == 'Tensor':
        ...
        h_gt, w_gt = img_gts[0].size()[-2:]
        h_mask, w_mask = img_masks[0].size()[-2:]
        h_label, w_label = img_labels[0].size()[-2:]
    else:
        h_lq, w_lq = img_lqs[0].shape[0:2]
        h_gt, w_gt = img_gts[0].shape[0:2]
        h_mask, w_mask = img_masks[0].shape[0:2]
        h_label, w_label = img_labels[0].shape[0:2]
    ...
    for _ in range(10000):
        ...
        # Crop corresponding label patch
        if input_type == 'Tensor':
            img_labels_cropped = [v[:, :, top_gt:top_gt + gt_patch_size, left_gt:left_gt + gt_patch_size] for v in img_labels]
        else:
            img_labels_cropped = [v[top_gt:top_gt + gt_patch_size, left_gt:left_gt + gt_patch_size, ...] for v in img_labels]
        ...
        if zero_ratio_mask < 0.20:
            ...
            best_crop_gt = img_gts_cropped
            best_crop_mask = img_masks_cropped
            best_crop_label = img_labels_cropped
            break
    ...
    if len(best_crop_mask) == 1:
        best_crop_mask = best_crop_mask[0]
    if len(best_crop_label) == 1:
        best_crop_label = best_crop_label[0]
    

    return best_crop_gt, best_crop_lq, best_crop_mask, best_crop_label